...
const EnhancedTableToolbar = ({ numSelected }: { numSelected: number }) => (
  ...
  >
    {numSelected > 0 ? (
      ...
      <Typography variant="h4" id="tableTitle"></Typography>
    )}
    <Box sx={{ flexGrow: 40 }} />
    {numSelected > 0 && (
...
function EnhancedTable() {
  const [order, setOrder] = React.useState<ArrangementOrder>("asc"),
    [orderBy, setOrderBy] = React.useState("calories"),
    ...
    handleRequestSort = (event: React.SyntheticEvent, property: string) => {
    ...
    isSelected = (OrganisationName: string) =>
      selected.indexOf(OrganisationName) !== -1,
    emptyRows =
      page > 0 ? Math.max(0, (1 + page) * rowsPerPage - rows.length) : 0;

  return (
    <MainCard content={false} title="Manage Tenants">
              ...
              .map((row, index) => {
                ...
                return (
                  <TableRow
                    hover
                    onClick={(event) =>
                      handleClick(event, row.OrganisationName)
                    }
                    role="checkbox"
